...
static int gen_alloc_exit2(void *ctx, u64 address)
{
	...
	if (address != 0) {
		...
		if (kernel_trace)
		{
			info.kern_stack_size = (int)bpf_get_stack(ctx, info.kern_stack, sizeof(info.kern_stack), 0);
		}
		info.user_stack_size = (int)bpf_get_stack(ctx, info.user_stack, sizeof(info.user_stack), BPF_F_USER_STACK);
...
int mem__kmalloc_node(void *ctx)
{
	const void *ptr;
	size_t bytes_alloc;
...
int mem__kmem_cache_alloc(void *ctx)
{
	const void *ptr;
	size_t bytes_alloc;
...
int mem__kmem_cache_alloc_node(void *ctx)
{
	const void *ptr;
	size_t bytes_alloc;
...
int mem__kmem_cache_free(void *ctx)
{
	const void *ptr;
...
int mem__mm_page_alloc(struct trace_event_raw_mm_page_alloc *ctx)
{
...
int mem__mm_page_free(struct trace_event_raw_mm_page_free *ctx)
{
	return gen_free_enter((void *)ctx->pfn);
}

SEC("tracepoint/percpu/percpu_alloc_percpu")
int mem__percpu_alloc_percpu(struct trace_event_raw_percpu_alloc_percpu *ctx)
{
...
int mem__percpu_free_percpu(struct trace_event_raw_percpu_free_percpu *ctx)
{
